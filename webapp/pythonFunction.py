...
def findDurability(itemType: str, itemName: str, raidType: str = 'explo', durabFile: str = r'data/rustlabsDurabilityData.json'):
    ...
    if raidType == 'explo':
        ...
        for key, value in dict_.items():
            if value[-1] == cheapest:
                return f"Trying to {raidType}raid:      {itemName}<br>Best option to {raidType}raid:      {key}<br>Cost:      {value[-1]} sulfur<br>Time to raid:      {value[3]}<br>Quantity needed:      {value[1]}"
    ...
    # If the raid type is eco, find the cheapest (time) item to raid with
        ...
        for key,value in dict_.items():
            if value[2] == cheapest:
                return f"Trying to {raidType}raid: {itemName}<br>Best option to {raidType}raid: {key}<br>Time to {raidType}raid: {value[3]}<br>Quantity needed: {value[1]}"
